...
function ConfigurationBrandingPreview({ logoUrl, legalName, taxId, lines, email, phone }) {
  return (
    <section className="configuration-card configuration-branding-card">
      <div className="configuration-card-heading">
        <div>
          <h4>Branding documental activo</h4>
          <p>Este bloque se reutiliza en la salida documental de cotizaciones.</p>
        </div>
      </div>
    ...
    <div className="configuration-module-grid">
      {items.map((item) => (
        <section key={item.title} className="configuration-card">
          ...
          {item.action === "audit" ? (
            <button type="button" className="btn-secondary" onClick={onOpenAudit}>
              Abrir historial
            </button>
          ) : null}
        </section>
...
function buildAddressPreviewLines(form, countryName) {
  ...
  const locality = [form.city, form.stateRegion].map((value) => String(value || "").trim()).filter(Boolean).join(", ");
  const localityLine = [
    locality,
    String(form.postalCode || "").trim() ? `CP ${String(form.postalCode || "").trim()}` : "",
  ]
    .filter(Boolean)
    .join(", ");
  ...
  if (!entries.length) {
    return (
      <div className="configuration-placeholder-card">
        ...
        <p>Cuando se modifique la configuracion, el historial aparecera aqui.</p>
      </div>
    );
  }
      ...
      {entries.map((entry) => (
        <article key={entry.id} className="configuration-audit-item">
          ...
          <div className="configuration-audit-item-body">
            ...
            <p>{entry.detail || summarizeChangedFields(entry.changed_fields)}</p>
          </div>
        </article>
      ))}
    </div>
...
export default function ConfigurationPage() {
  ...
  const countryName = useMemo(
    () =>
      countries.find((country) => String(country.id) === String(form.countryId))
        ?.name || companyProfile?.countryName || "",
  ...
  const requiredFieldItems = useMemo(
    () => [
      ...
      },
      {
        ...
        description: "Identificador fiscal requerido para cotizaciones y referencias.",
        complete: Boolean(String(form.taxId || "").trim()),
      },
      {
        label: "Direccion principal",
        description: "Base del domicilio institucional que se imprime en documentos.",
      ...
      {
        label: "Codigo postal",
        description: "Se agrega al domicilio institucional y validaciones operativas.",
  ...
  const optionalFieldItems = useMemo(
    () => [
      ...
      },
      {
        ...
        description: "Canal de contacto visible para clientes y equipos internos.",
        complete: Boolean(String(form.email || "").trim()),
      },
      {
        label: "Telefono institucional",
        description: "Referencia operativa visible en la documentacion emitida.",
      ...
      },
      {
        ...
        description: "Contexto interno del perfil de empresa para administradores.",
  ...
  const requiredCompleted = requiredFieldItems.filter((item) => item.complete).length;
  const optionalCompleted = optionalFieldItems.filter((item) => item.complete).length;

  const summaryItems = useMemo(
    () => [
      { label: "Nombre comercial", value: form.commercialName || "Sin definir" },
    ...
    [countryName, form.commercialName, form.email, form.phone, form.website, latestUpdateText],
  );
  ...
  const moduleItems = useMemo(
    ...
    [auditEntries.length, brandingLines.length, form.legalName, form.taxId, formatDateTime, latestAuditEntry],
  );
    ...
    <section className="panel configuration-page">
      <header className="configuration-header">
        <div>
          <div className="module-title-with-icon">
            <h2>Configuracion</h2>
            <span className="module-title-icon configuration-title-icon" aria-hidden="true">
          ...
          <p className="roles-subtitle">
            Administra los datos institucionales y parametros globales de la aplicacion.
          </p>
      ...
      <div className="configuration-layout">
        <aside className="configuration-sidebar">
          <div className="configuration-sidebar-title">Configuracion general</div>
            ...
            <div className="configuration-company-grid">
                ...
                <div className="configuration-form-grid">
                  <div className="field-group">
                    ...
                    <p className="field-hint">
                      Se usara en documentos oficiales y encabezados institucionales.
                    </p>
                  </div>
                  <div className="field-group">
              ...
              <section className="configuration-card">
                ...
                <div className="configuration-logo-card">
                  <div className="configuration-logo-preview">
                    {form.logoUrl ? (
                      <img src={form.logoUrl} alt="Vista previa del logo institucional" />
                    ) : (
                      <div className="configuration-logo-empty">Sin logo cargado</div>
                    )}
                  </div>
              ...
              <section className="configuration-card">
                ...
                <div className="configuration-form-grid">
                  <div className="field-group configuration-grid-span-full">
                    <label>
                      Direccion principal <span className="required-mark">*</span>
                    </label>
                    <input
                      type="text"
              ...
              <section className="configuration-card">
                <div className="configuration-card-heading">
                  <div>
                    ...
                    <p>Vista consolidada del perfil que hoy consume la aplicacion.</p>
                  </div>
                  <span className="configuration-inline-pill">
                    {formatCompletionPercent(requiredCompleted, requiredFieldItems.length)}%
                  </span>
                </div>

                <div className="configuration-metrics-grid">
                  <article className="configuration-metric-card">
                    <strong>{formatCompletionLabel(requiredCompleted, requiredFieldItems.length)}</strong>
                    <span>Campos obligatorios</span>
                  </article>
                  <article className="configuration-metric-card">
                    <strong>{formatCompletionLabel(optionalCompleted, optionalFieldItems.length)}</strong>
              ...
              <section className="configuration-card">
                <div className="configuration-card-heading">
                  <div>
                    ...
                    <p>
                      Esta configuracion ya tiene consumidores claros y deja visible su alcance real.
                    </p>
                  </div>
                </div>
              </section>
