...
(async () => {
  try {
    ...
    // Get all won opportunities for Valeria with full details
    const results = await query(`
      ...
      WHERE u.full_name = 'Valeria Moulinie'
    ...
    console.log('\n' + '='.repeat(120));
    console.log('DESGLOSE DETALLADO: VALERIA MOULINIE - CÁLCULO O→V');
    console.log('='.repeat(120) + '\n');

    if (!results.length) {
      console.log('No oportunidades ganadas encontradas para Valeria Moulinie');
      return;
    }
    ...
    if (daysArray.length > 0) {
      ...
      console.log(`\n   ✅ TIEMPO O→V PROMEDIO DE VALERIA: ${avgDays} días`);
    }
