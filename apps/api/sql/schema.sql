...
CREATE TABLE IF NOT EXISTS contacts (
  ...
  hierarchy_level_id BIGINT UNSIGNED NOT NULL,
  relationship_type_id BIGINT UNSIGNED NOT NULL,
  influence_level_id BIGINT UNSIGNED NOT NULL,
  ...
  CONSTRAINT fk_contacts_hierarchy_level FOREIGN KEY (hierarchy_level_id) REFERENCES contact_hierarchy_levels(id),
  CONSTRAINT fk_contacts_relationship_type FOREIGN KEY (relationship_type_id) REFERENCES contact_relationship_types(id),
  CONSTRAINT fk_contacts_influence_level FOREIGN KEY (influence_level_id) REFERENCES contact_influence_levels(id),
...
SET @stmt := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE contacts ADD COLUMN hierarchy_level_id BIGINT UNSIGNED NULL AFTER purchase_participation_id',
    'SELECT 1'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
...
PREPARE s_contacts_hierarchy_level_col FROM @stmt;
EXECUTE s_contacts_hierarchy_level_col;
DEALLOCATE PREPARE s_contacts_hierarchy_level_col;

SET @stmt := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE contacts ADD COLUMN influence_level_id BIGINT UNSIGNED NULL AFTER relationship_type_id',
    'SELECT 1'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
...
PREPARE s_contacts_influence_level_col FROM @stmt;
EXECUTE s_contacts_influence_level_col;
DEALLOCATE PREPARE s_contacts_influence_level_col;

SET @stmt := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE contacts ADD CONSTRAINT fk_contacts_hierarchy_level FOREIGN KEY (hierarchy_level_id) REFERENCES contact_hierarchy_levels(id)',
    'SELECT 1'
  )
  FROM information_schema.TABLE_CONSTRAINTS
  WHERE CONSTRAINT_SCHEMA = DATABASE()
    AND TABLE_NAME = 'contacts'
    AND CONSTRAINT_NAME = 'fk_contacts_hierarchy_level'
);
PREPARE s_contacts_hierarchy_level_fk FROM @stmt;
EXECUTE s_contacts_hierarchy_level_fk;
DEALLOCATE PREPARE s_contacts_hierarchy_level_fk;

SET @stmt := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE contacts ADD CONSTRAINT fk_contacts_influence_level FOREIGN KEY (influence_level_id) REFERENCES contact_influence_levels(id)',
    'SELECT 1'
  )
  FROM information_schema.TABLE_CONSTRAINTS
  WHERE CONSTRAINT_SCHEMA = DATABASE()
    AND TABLE_NAME = 'contacts'
    AND CONSTRAINT_NAME = 'fk_contacts_influence_level'
);
PREPARE s_contacts_influence_level_fk FROM @stmt;
EXECUTE s_contacts_influence_level_fk;
DEALLOCATE PREPARE s_contacts_influence_level_fk;
...
VALUES
  ...
  ('comercial.seller.eligible', 'comercial', 'seller_eligible', 'Usuario elegible para ser asignado como vendedor', NOW(3), NOW(3)),
...
INSERT INTO contact_hierarchy_levels (code, name, is_active) VALUES
  ('director', 'Director', 1),
  ('gerente', 'Gerente', 1),
  ('lider', 'Líder', 1),
...
INSERT INTO contact_influence_levels (code, name, is_active) VALUES
  ('baja', 'Baja', 1),
  ('media', 'Media', 1),
  ('alta', 'Alta', 1)
ON DUPLICATE KEY UPDATE name = VALUES(name), is_active = VALUES(is_active);
