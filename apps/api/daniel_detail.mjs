...
(async () => {
  try {
    ...
    // Get all won opportunities for Daniel with full details
    const results = await query(
      ...
      WHERE u.full_name = 'Daniel Saucedo'
    ...
    console.log("\n" + "=".repeat(120));
    console.log("DESGLOSE DETALLADO: DANIEL SAUCEDO - CÁLCULO O→V");
    console.log("=".repeat(120) + "\n");

    if (!results.length) {
      console.log("No oportunidades ganadas encontradas para Daniel Saucedo");
      return;
    }
    ...
    results.forEach((row, idx) => {
      ...
      if (row.dev_entered_date) {
        ...
        const days = Math.max(
          0,
          Math.floor((closedDate - devDate) / (1000 * 60 * 60 * 24)),
        );

        console.log(
          `   📅 Entró en "desarrollo": ${devDate.toLocaleDateString("es-ES")} (${devDate.toLocaleTimeString("es-ES")})`,
        );
        console.log(
          `   📅 Cerrada como venta:    ${closedDate.toLocaleDateString("es-ES")} (${closedDate.toLocaleTimeString("es-ES")})`,
        );
        console.log(`   ⏱️  DÍAS EN DESARROLLO: ${days} días`);
      } else {
        console.log(
          `   ⚠️  Sin registro en auditoría de entrada a "desarrollo"`,
        );
        console.log(
          `   📅 Cerrada como venta: ${new Date(row.closed_date).toLocaleDateString("es-ES")}`,
        );
      }
    });
    ...
    if (daysArray.length > 0) {
      ...
      console.log(`\n🎯 CÁLCULO FINAL:`);
      console.log(
        `   Días totales: ${daysArray.join(" + ")} = ${daysArray.reduce((sum, d) => sum + d, 0)} días`,
      );
      console.log(`   Total oportunidades con auditoría: ${daysArray.length}`);
      console.log(
        `   Promedio: ${daysArray.reduce((sum, d) => sum + d, 0)} ÷ ${daysArray.length} = ${avgDays} días`,
      );
      console.log(`\n   ✅ TIEMPO O→V PROMEDIO DE DANIEL: ${avgDays} días`);
    } else {
      console.log(`\n${"─".repeat(120)}`);
      console.log(`\n⚠️  CÁLCULO FINAL:`);
      console.log(
        `   Daniel tiene oportunidades ganadas (${results.length}), pero NINGUNA tiene registro en auditoría`,
      );
      console.log(`   de entrada a etapa "desarrollo"`);
      console.log(
        `\n   Por política de cálculo, se usa 120 días como valor por defecto para cada oportunidad:`,
      );
      const defaultTotal = results.length * 120;
      const avgDays = Math.round(defaultTotal / results.length);
      console.log(
        `   Cálculo: (${results.length} opp × 120 días) ÷ ${results.length} = ${avgDays} días`,
      );
      console.log(`\n   ✅ TIEMPO O→V PROMEDIO DE DANIEL: ${avgDays} días`);
    }
