  ...
  try {
    ...
    resultBySeller.forEach((data, sellerId) => {
      ...
      sellerSummary.push({
        ...
        min_days: data.days_array.length > 0 ? Math.min(...data.days_array) : null,
        max_days: data.days_array.length > 0 ? Math.max(...data.days_array) : null
      });
    });
    ...
    sellerSummary.forEach((s, idx) => {
      const status = s.average_days <= 30 ? '🔥' : s.average_days <= 60 ? '⚡' : '⏳';
      console.log(`${status} ${String(idx + 1).padStart(2)}. ${s.seller_name.padEnd(25)} | ${String(s.average_days).padStart(3)} días promedio | ${s.with_audit_trail}/${s.total_won} opp. ganadas`);
    });
    ...
    resultBySeller.forEach((data, sellerId) => {
      ...
      console.log(`   Rango: ${data.days_array.length > 0 ? `${Math.min(...data.days_array)} a ${Math.max(...data.days_array)} días` : 'N/A'}`);
